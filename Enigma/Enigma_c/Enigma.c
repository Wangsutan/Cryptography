...
int main(int argc, char** argv)
{
    ...
    } else if (passwordFrom == 'M')
    {
        ...
        for (int i = 0; i < rotorsNum * passwordLen; i++)
            passwords[i / passwordLen][i % passwordLen] = password_unit[i];
    }
    ...
    for (int i = 0; i < rotorsNum; i++)
    {
        ...
        for (int j = 0; j < passwordLen; j++)
        {
            rotorList[i].order[j] = passwords[i][j];
        }
    }
    ...
    for (int i = 0; i < alphabetLen; i++)
        reflection[i / (alphabetLen / 2)][i % (alphabetLen / 2)] = reflection_unit[i];
    ...
    for (i = 0; i < plainTextLen; i++)
    {
        ...
        reflect(*reflection, reflectionLen, plainText, i);
...
void reflect(int *reflection, int reflectionLen, char plainText[], int i)
{
    ...
    for (j = 0; j < 2; j++)
    {
        for (k = 0; k < reflectionLen / 2; k++)
        {
            if ( index_orig == reflection[j * (reflectionLen / 2) + k] )
            {
                int index_new = reflection[(1 - j) * (reflectionLen / 2) + k];
